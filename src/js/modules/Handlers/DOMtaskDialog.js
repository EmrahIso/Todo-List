...
const DOMTaskDialog = (function () {
  ...
  const openTaskDetailsBtnClickEventHandler = (e) => {
    ...
    const taskPropertiesObj = getPropertiesOfSpecificProjectsTask(
      projectName,
      taskName,
    );

    dialogEl.querySelector('[data-dialog-task-name]').textContent =
      `// ${taskPropertiesObj.title}`;
    dialogEl
      .querySelector('[data-dialog-task-name]')
      .setAttribute('data-dialog-task-name', taskPropertiesObj.title);
    ...
    dialogEl.querySelectorAll('[data-task-dialog-property]').forEach((n) => {
      ...
      n.querySelector('[data-task-dialog-property-value]').value =
        taskPropertiesObj[dataAttrValue];

      if (
        n
          .querySelector('[data-task-dialog-property-value]')
          .getAttribute('id') === 'dialogTaskChecklist'
      ) {
    ...
    dialogEl
      .querySelectorAll('[data-task-dialog-property-value]')
      .forEach((n) => {
  ...
  const openTaskEditBtnClickEventHandler = (e) => {
    ...
    dialogEl.querySelector('[data-task-dialog-save]').style.display =
      'inline-block';
  ...
  const taskDialogFormSubmitEventHandler = (e) => {
    ...
    const taskName = e.target.previousElementSibling.getAttribute(
      'data-dialog-task-name',
    );
    ...
    dialogEl.querySelectorAll('[data-task-dialog-property]').forEach((n) => {
      ...
      if (dataAttrValue == 'checklist') {
        ...
        newValueTaskData[dataAttrValue] = n.querySelector(
          '[data-task-dialog-property-value]',
        ).value;
      }
    });
    ...
    getTaskManagerControl()
      ...
      .changeTaskKey(taskName, newValueTaskData.title);
