...
function UIRenderProjects(allProjects) {
    ...
    const dynamicProjectsNavigationListEl = document.querySelector("[data-nav-projects] [data-dynamic-projects-cont]");
    ...
    allProjectsArray.forEach(project => {
        ...
        dynamicProjectsNavigationListEl.appendChild(dynamicProjectNavigationListItemEl);

        const projectButtonEl = document.createElement("button");
        projectButtonEl.classList.add("button", "button--navigation", "button--navigation-project");
        ...
        projectButtonDeleteButtonEl.classList.add("button", "button--navigation", "button--navigation-delete");
        ...
        if(document.querySelector(`[data-tasks-content] [data-project-id="${project.name}"]`) != null) {
            return
        } else {
...
function UISwitchProjects(activeProject) {
    ...
    document.querySelector("[data-active-project-heading]").textContent = activeProject.name;

    const projectTaskContainerEls = document.querySelectorAll(".task__board div[data-project-id]");
